...
// SCENARIO HELPERS
// -------------------------
function getRandomScenariosByCategory() {
  const categories = ["AP", "Branch", "DataCenter", "Firmware"];
  ...
  for (const category of categories) {
    ...
    const scenario = stmt.get(category);

    if (scenario) {
      selectedScenarios.push({
        ...scenario,
        required_ports: JSON.parse(scenario.required_ports),
      });
    }
  }
...
function getScenarioById(scenarioId) {
  ...
  const scenario = stmt.get(scenarioId);
...
function createScenarioRun(runId, scenarioId, scenarioOrder) {
  const stmt = db.prepare(`
    INSERT INTO scenario_runs (run_id, scenario_id, scenario_order, result, time_ms)
    VALUES (?, ?, ?, 'pending', 0)
  `);

  const info = stmt.run(runId, scenarioId, scenarioOrder);
...
function completeScenarioRun(scenarioRunId, result, timeMs, penaltyApplied = 0) {
  const stmt = db.prepare(`
    UPDATE scenario_runs
    SET result = ?, time_ms = ?, penalty_applied = ?
    WHERE id = ?
  `);

  stmt.run(result, timeMs, penaltyApplied ? 1 : 0, scenarioRunId);

  return db
    .prepare(`
      SELECT sr.*, s.name, s.category, s.required_ports
...
function getScenarioRunsByRunId(runId) {
  return db
    .prepare(`
      SELECT sr.*, s.name, s.category, s.required_ports
      FROM scenario_runs sr
      JOIN scenarios s ON s.id = sr.scenario_id
      WHERE sr.run_id = ?
      ORDER BY sr.scenario_order ASC
    `)
    .all(runId);
}

function updateRunWithPenalty(runId, penaltyMs) {
  const stmt = db.prepare(`
    UPDATE runs
    SET penalty_ms = penalty_ms + ?
    WHERE id = ?
  `);

  stmt.run(penaltyMs, runId);
